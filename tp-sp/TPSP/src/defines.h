...
#define GDT_DATA_0_SEL (GDT_IDX_DATA_0 << 3)    /*esto es lo que habia explicado furfa de que el segmento se encuentra en */
#define GDT_CODE_3_SEL ((GDT_IDX_CODE_3 << 3) | 3)             /*(indice de segmento*8) + algo que no me acuerdo que era*/
#define GDT_DATA_3_SEL ((GDT_IDX_DATA_3 << 3) | 3)/*recordar que cada descriptor mide 64 bits (=8 bytes) y por eso hay que multiplicar por 8*/
